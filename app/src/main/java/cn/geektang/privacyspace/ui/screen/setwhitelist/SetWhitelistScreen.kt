...
fun SetWhitelistScreen(viewModel: SetWhitelistViewModel = viewModel()) {
    ...
    SetWhitelistContent(appInfoList = appInfoList, whitelist = whitelist, isLoading = isLoading,
...
@Composable
fun SetWhitelistContent(appInfoList: List<AppInfo>, whitelist: Set<String>, isLoading: Boolean,
    onAppCheckedChange: (AppInfo, Boolean) -> Unit
) {
    Column {
        val navController = LocalNavHostController.current
        TopBar(title = stringResource(R.string.set_whitelist),
...
fun SetWhitelistScreenPreview() {
    ...
    SetWhitelistContent(listOf(data, data), setOf(BuildConfig.APPLICATION_ID), false, { _, _ -> })
}
