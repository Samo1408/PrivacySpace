...
class HookMain : IXposedHookLoadPackage {
        ...
        fun updateConfigData(data: ConfigData) {
            ...
            if (!sharedUserIdMap.isNullOrEmpty()) {
                ...
                connectedAppsTpm.forEach {
                    ...
                    it.value.forEach { pName ->
                        val sharedUserId = sharedUserIdMap[pName]
    ...
    override fun handleLoadPackage(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        if (lpparam.packageName == ConfigConstant.ANDROID_FRAMEWORK) {
            ...
            when {
                Build.VERSION.SDK_INT >= 30 -> FrameworkHookerApi30Impl.start(classLoader)
                Build.VERSION.SDK_INT >= 28 -> FrameworkHookerApi28Impl.start(classLoader)
                else -> FrameworkHookerApi26Impl.start(classLoader)
            }
                ...
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        MainScope().launch {
                            ...
                            if (null != config) updateConfigData(config)
                        }
                    }
                })
    ...
    private fun startWatchingConfigFiles() {
        if (null == fileObserver) {
            File(ConfigConstant.CONFIG_FILE_FOLDER).mkdirs()
            fileObserver = object : FileObserver(ConfigConstant.CONFIG_FILE_FOLDER) {
    ...
    private fun loadConfigDataAndParse() {
        val newConfig = ConfigHelper.loadConfigWithSystemApp(packageName ?: "") ?: ConfigData.EMPTY
        if (newConfig != configData) {
            configData = newConfig
            updateConfigData(newConfig)
        }
    }
}
