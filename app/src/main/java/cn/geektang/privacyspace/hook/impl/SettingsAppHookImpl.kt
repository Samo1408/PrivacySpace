...
object SettingsAppHookImpl : Hooker, XC_MethodHook() {
    ...
    override fun start(classLoader: ClassLoader) {
        ...
        } catch (e: ClassNotFoundException) { XLog.e(e, "SettingsAppHookImpl start failed."); return }
        ...
        XposedHelpers.findAndHookMethod(UsageStatsManager::class.java, "queryUsageStats",
            Int::class.javaPrimitiveType, Long::class.javaPrimitiveType, Long::class.javaPrimitiveType, this)
        ...
        when (param.method.name) {
            "getInstalledPackages" -> {
                ...
                while (iterator.hasNext()) {
                    if (hiddenAppList.contains(iterator.next().packageName)) iterator.remove()
                }
            }
            ...
            "queryIntentActivities" -> {
                ...
                val iterator = result.list.iterator()
                while (iterator.hasNext()) {
                    if (hiddenAppList.contains(iterator.next().getPackageName())) iterator.remove()
                }
            }
            "queryUsageStats" -> {
            ...
            "getRunningAppProcesses" -> {
                param.result = (param.result as? List<*>?)?.filter {
                    ...
                    !it.pkgList.any { pkg -> hiddenAppList.contains(pkg) }
                }
            }
        }
    }
